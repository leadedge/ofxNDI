	...
	bool bSwapRB, bool bInvert)
{
	...
	if (pNDI_send && bSenderInitialized && pixels && width > 0 && height > 0) {
		...
		// LJ DEBUG
		if (m_Format == NDIlib_FourCC_video_type_UYVY) {

			ofxNDIutils::StartTiming();

			if (!m_p_uyvy)
				m_p_uyvy = (BYTE *)malloc(width*height * 4 * sizeof(unsigned char));
			if (!m_oldFrame) {
				m_oldFrame = (BYTE *)malloc(width*height * 4 * sizeof(unsigned char));
				memset((void *)m_oldFrame, 0, width*height * 4);
			}
			// printf("m_p_uyvy = 0x%.7X\n", PtrToUint(m_p_uyvy));

			const bool bChanges = libloader.bgrx_to_uyvy_cond(
				pixels, width * 4, // new source
				m_oldFrame, width * 4, // old source
				m_p_uyvy, width * 2, // YUV buffer
				width, height); // dimensions

			// printf("changes = %d\n", bChanges);
			if (m_p_uyvy) {
				video_frame.line_stride_in_bytes = width * 2;
				video_frame.p_data = (uint8_t*)m_p_uyvy;
			}
			else {
				return false;
			}
	...
	unsigned int sourcePitch, bool bInvert)
{
	...
	if (pNDI_send && bSenderInitialized && pixels && width > 0 && height > 0) {
		...
		if (m_Format == NDIlib_FourCC_video_type_UYVY) {
			...
			if(!m_p_uyvy) m_p_uyvy = (BYTE *)malloc(width*height*2*sizeof(unsigned char));
			const bool bChanges = libloader.bgrx_to_uyvy_cond(
											pixels, width*4, // new source
											NULL, NULL, // old source
											m_p_uyvy, width*2, // YUV buffer
											width, height); // dimensions

			if (m_p_uyvy) {
				video_frame.p_data = (uint8_t*)m_p_uyvy;
				printf("m_p_uyvy = 0x%.7X, changes = %d\n", PtrToUint(m_p_uyvy), bChanges);
			}
			else {
				return false;
			}
