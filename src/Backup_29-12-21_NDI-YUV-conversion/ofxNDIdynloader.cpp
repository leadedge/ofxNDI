...
const NDIlib_v4* ofxNDIdynloader::Load()
{
	...
	*((FARPROC*)&NDIlib_v4_load) = GetProcAddress(m_hNDILib, "NDIlib_v4_load");
	...
	// LJ DEBUG
	// Get the address of the BGRX to UYVY with conditional change support
	*(FARPROC*)&bgrx_to_uyvy_cond = GetProcAddress(m_hNDILib, "bgrx_to_uyvy_cond");

	printf("bgrx_to_uyvy_cond = 0x%.7X\n", PtrToUint(bgrx_to_uyvy_cond));
