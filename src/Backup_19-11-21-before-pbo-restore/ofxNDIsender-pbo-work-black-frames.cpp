/*
	...
	18.11.21 - Revise ReadTexturePixels further due to glBufferStorage
			   extension missing for MacOS
...
bool ofxNDIsender::CreateSender(const char *sendername, unsigned int width, unsigned int height)
{
	...
	if (m_fbo == 0) {
		printf("Create sender - create fbo\n");
...
bool ofxNDIsender::SendImage(ofTexture tex, bool bInvert)
{
	...
	// LJ DEBUG
	// printf("m_idx = %d\n", m_idx);
	if (!ReadPixels(tex, width, height, ndiBuffer[m_idx])) {
		// LJ DEBUG
		printf("ReadPixels failed\n");
		return false;
	}
	else {
		// LJ DEBUG
		unsigned char * npix = ndiBuffer[m_idx].getData();
		npix += width * 4;
		unsigned char pix0 = npix[0];
		unsigned char pix1 = npix[1];
		unsigned char pix2 = npix[2];
		unsigned char pix3 = npix[3];

		unsigned char pix4 = npix[4];
		unsigned char pix5 = npix[5];
		unsigned char pix6 = npix[6];
		unsigned char pix7 = npix[7];

		unsigned char pix8 = npix[8];
		unsigned char pix9 = npix[9];
		unsigned char pix10 = npix[10];
		unsigned char pix11 = npix[11];
		...
		if (pix0 == 0
			...
			// printf("0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X 0x%2.2X\n",
			printf("%d %d %d %d %d %d %d %d %d %d %d %d\n",
				pix0, pix1, pix2, pix3, pix4, pix5, pix6, pix7, pix8, pix9, pix10, pix11);
		}
...
{
	// LJ DEBUG
	// printf("m_bReadback = %d\n", m_bReadback);

	if (m_bReadback) {
		// LJ DEBUG
		// return ReadTexturePixels(tex, width, height, buffer.getData());
		if (!ReadTexturePixels(tex, width, height, buffer.getData())) {
			// LJ DEBUG
			printf("ReadTexturePixels failed\n");
...
bool ofxNDIsender::ReadTexturePixels(ofTexture tex, unsigned int width, unsigned int height, unsigned char *data, GLuint HostFBO)
{
	...
	if (data == NULL) {
		printf("ReadTexturePixels : No data\n");
		return false;
	}

	if (m_fbo == 0) {
		printf("ReadTexturePixels : create fbo\n");
		glGenFramebuffers(1, &m_fbo);
	}
	...
	if (m_pbo[0] == 0) {
		printf("ReadTexturePixels : create 3 pbos\n");
...
// Black frames
bool ofxNDIsender::ReadTexturePixels2(ofTexture tex, unsigned int width, unsigned int height, unsigned char *data, GLuint HostFBO)
{
	void *pboMemory = nullptr;
