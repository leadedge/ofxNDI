	...
	if (pNDI_send) {
		...
		*/
		...
		// if (m_Format == NDIlib_FourCC_video_type_UYVY)
			// video_frame.line_stride_in_bytes = width * 2; // YUV422 line stride
		// else
...
bool ofxNDIsend::UpdateSender(unsigned int width, unsigned int height)
{
	...
	// if (m_Format == NDIlib_FourCC_video_type_UYVY)
		// video_frame.line_stride_in_bytes = width * 2; // YUV422
	// else
		video_frame.line_stride_in_bytes = (int)width * 4; // RGBA/BGRA
