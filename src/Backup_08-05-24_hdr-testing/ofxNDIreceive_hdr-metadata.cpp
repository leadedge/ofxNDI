...
bool ofxNDIreceive::CreateReceiver(int userindex)
{
	...
	return CreateReceiver(NDIlib_recv_color_format_best, userindex);
/*
#if defined(_WIN64)
			...
			case NDIlib_frame_type_video :
				...
				if (video_frame.p_data) {
					...
					if (video_frame.p_metadata) {
						...
						// <ndi_color_info primaries="bt_2100" transfer="bt_2100_hlg" matrix="bt_2100" />
						std::string info = video_frame.p_metadata;
						std::string transfer;
						std::string matrix;
						std::string primaries;
						// Is there "ndi_color_info"
						size_t pos = info.find("ndi_color_info");
						if (pos != std::string::npos) {
							// Attributes
							info = info.substr(pos);
							// Primaries
							pos = info.find("primaries");
							if (pos != std::string::npos) {
								primaries = info.substr(pos+11);
								primaries = primaries.substr(0, primaries.find(" ")-1);
							}
							// Transfer
							pos = info.find("transfer");
							if (pos != std::string::npos) {
								transfer = info.substr(pos+10);
								transfer = transfer.substr(0, transfer.find(" ")-1);
							}
							// Matrix
							pos = info.find("matrix");
							if (pos != std::string::npos) {
								matrix = info.substr(pos+8);
								matrix = matrix.substr(0, matrix.find(" ")-1);
							}
							printf("%s\n%s\n%s\n%s\n%s\n",
								senderName.c_str(),	video_frame.p_metadata,
								transfer.c_str(), matrix.c_str(), primaries.c_str());
							MessageBoxA(NULL, "xml data", "receive", MB_OK);
						}
					}
