...
void ofxNDIsender::SetFormat(NDIlib_FourCC_video_type_e format)
{
	if (format == NDIlib_FourCC_video_type_UYVY) {
		// Find the rgba2yuv shader fragment file full path
		std::string shaderpath = ofFilePath::getCurrentExeDir();
#ifdef TARGET_OPENGLES
		shaderpath += "data\\rgba2yuv\\ES2\\rgba2yuv.frag";
#else
		if (ofIsGLProgrammableRenderer())
			shaderpath += "data\\rgba2yuv\\GL3\\rgba2yuv.frag";
		else
			shaderpath += "data\\rgba2yuv\\GL2\\rgba2yuv.frag";
#endif
		// Test existence of required rgba2yuv shader file
		if (ofFile::doesFileExist(shaderpath, false)) { // full file path
...
bool ofxNDIsender::ReadYUVpixels(ofTexture &tex, unsigned int halfwidth, unsigned int height, ofPixels &buffer)
{
	...
	if (!rgba2yuv.isLoaded()) {
		// Find the rgba2yuv shader folder full path
		std::string shaderpath = ofFilePath::getCurrentExeDir();
#ifdef TARGET_OPENGLES
