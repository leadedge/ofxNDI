...
ofxNDIreceive::ofxNDIreceive()
{
	...
	// Initialize video frame timestamp, timecode and interval
			...
			case NDIlib_frame_type_video:
				...
				if (video_frame.p_data) {
					...
					// Otherwise sizes are current - copy the received frame data to the local buffer
						...
						// Get the current video frame timestamp
						uint64_t timestamp = video_frame.timestamp;

						// Calculate time since the last frame in milliseconds
						m_VideoFrameTime = (double)(timestamp - m_VideoTimestamp) / 10000.0;
