...
bool ofxNDIsend::CreateSender(const char *sendername, unsigned int width, unsigned int height)
{
	...
	if (pNDI_send) {
		...
		*/
		...
		// Line stride in bytes depending on format
...
bool ofxNDIsend::UpdateSender(unsigned int width, unsigned int height)
{
	...
	// printf("UpdateSender %dx%d, format = %d, stride = %d\n", width, height, m_Format, video_frame.line_stride_in_bytes);
...
{
	m_Format = format;
	// Reset the format and line stride if a sender has been created
	if (bSenderInitialized) {
		SetFormat(m_Format);
		SetVideoStride(m_Format);
	}
...
// Set video frame line stride in bytes allowing for half width YUV
// Uses the global variable "video_frame"
// Must be done after the frame dimensions are set
void ofxNDIsend::SetVideoStride(NDIlib_FourCC_video_type_e format)
{
	// LJ DEBUG
	if (format == NDIlib_FourCC_video_type_UYVY || format == NDIlib_FourCC_type_UYVA)
