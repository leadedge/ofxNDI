...
bool ofxNDIsend::CreateSender(const char *sendername, unsigned int width, unsigned int height)
{
	...
	if (pNDI_send) {
		...
		*/
		...
		// Line stride
		SetVideoStride(video_frame, m_Format, width);
	...
	bool bSwapRB, bool bInvert)
{
	...
	// LJ DEBUG
	// printf("SendImage bSwaprgb	%dx%d - format = %d\n", width, height, m_Format);
	// SetVideoStride(m_Format, width);
/*
// {
	video_frame.FourCC = m_Format;
	if (m_Format == NDIlib_FourCC_video_type_UYVY) {
		// LJ DEBUG
		printf("SendImage bSwaprgb	%dx%d - format = YUV\n", width, height);
...
// Set video frame line stride allowing for half width YUV
void ofxNDIsend::SetVideoStride(NDIlib_video_frame_v2_t videoframe, NDIlib_FourCC_video_type_e format, unsigned int framewidth)
{
	// Line stride in bytes
	if (format == NDIlib_FourCC_video_type_UYVY)
		videoframe.line_stride_in_bytes = framewidth * 2;
	else
		videoframe.line_stride_in_bytes = framewidth * 4;
