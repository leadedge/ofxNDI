...
{
	// LJ DEBUG
	printf("ofxNDIreceive::CreateFinder() - bNDIinitialized = %d\n", bNDIinitialized);
	...
	if (pNDI_find) {
		...
		if (p_sources && nsources != no_sources) {
			...
			if (!senderName.empty()) {
				...
				if (!senderName.empty()) {
					// LJ DEBUG
					printf("FindSenders : senderName [%s]\n", senderName.c_str());
				}

				if (NDIsenders.size() > 0) {
					// Reset the current sender index
					// LJ DEBUG
					printf("FindSenders : Reset the current sender index (%d)- bSenderSelected = %d\n", senderIndex, bSenderSelected);
					senderIndex = 0;
				...
				// LJ DEBUG
				printf("FindSenders : Sender index reset to (%d)\n", senderIndex);
				...
				if (senderName != NDIsenders.at(senderIndex)) {
					// LJ DEBUG
					printf("FindSenders : New sender at index %d [%s]\n", senderIndex, NDIsenders.at(senderIndex).c_str());
					bSenderSelected = true;
				}
				else {
					// LJ DEBUG
					printf("FindSenders : same sender [%s]\n", NDIsenders.at(senderIndex).c_str());
				}
...
bool ofxNDIreceive::SetSenderIndex(int index)
{
	if (!bNDIinitialized) {
		// LJ DEBUG
		printf("SetSenderIndex(%d) - not initialized\n", index);
		return false;
	}

	// LJ DEBUG
	printf("SetSenderIndex(%d) - current = %d\n", index, senderIndex);

	if (NDIsenders.empty()) {

		// If no finder, create one
		if (!pNDI_find) {
			CreateFinder();
		}

		// Find all the sources with a 4 second timeout in case of connection trouble.
		if (pNDI_find) {
		...
		printf("    found %d senders\n", NDIsenders.size());
	}

	// Change to the requested index
	// LJ DEBUG
	printf("    changing index from %d to %d\n", index, senderIndex);
	senderIndex = index;

	if (!NDIsenders.empty()) {

		// If it exceeds the sender list size return fasle with no action
		// The receiver will wait for the sekected index to appear in the list
		if (senderIndex >= (int)NDIsenders.size()) {
			printf("    Index %d exceeds sender list size (%d)\n", senderIndex, (int)NDIsenders.size());
			// senderIndex = (int)NDIsenders.size() - 1;
			return false;
		}

		// Return for the same sender
		if (!senderName.empty() && NDIsenders.at(senderIndex) == senderName) {
			printf("    Same sender\n");
			return false;
		}
		...
		printf("Sender at %d [%s]\n", senderIndex, senderName.c_str());
...
bool ofxNDIreceive::CreateReceiver(NDIlib_recv_color_format_e colorFormat , int userindex)
{
	...
	if (!pNDI_recv) {
		...
		if (p_sources && no_sources > 0) {
			...
			// This can happen if the index is set in advance. No receiver is created
			// and the receiver will wait for the nominated sender to open.
			if (userindex > (int)no_sources - 1) {
				// LJ DEBUG
				printf("CreateReceiver : index %d exceeds the number of sources (%d)\n", userindex, no_sources);
				return false;
			}
			...
			// LJ DEBUG
			printf("CreateReceiver : using index %d\n", index);
			...
			// Quit now if the index has been set greater than the sender list size
			// A sender index has been specified greater than the sender list size so  wait for it
			if (index >= (int)NDIsenders.size()) {
				// LJ DEBUG
				printf("Index %d exceeds sender list size %d\n", index, (int)NDIsenders.size());
				return false;
			}
			...
			// LJ DEBUG
			printf("CreateReceiver : Reset the current index value from %d to %d\n", index, senderIndex);
...
bool ofxNDIreceive::ReceiveImage(unsigned int &width, unsigned int &height)
{
	...
	// LJ DEBUG
	// printf("ReceiveImage 1\n");

	if (pNDI_recv) {

		// LJ DEBUG
		// printf("ReceiveImage 2\n");
