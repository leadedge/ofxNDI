...
// Sender video frame line stride in bytes
unsigned int ofxNDIreceive::GetSenderStride() {
	return m_Stride;
}
			...
			case NDIlib_frame_type_video:
				...
				if (video_frame.p_data) {
					...
					if (m_Width != (unsigned int)video_frame.xres || m_Height != (unsigned int)video_frame.yres) {
						...
						width = m_Width;
						height = m_Height;
						m_Stride = (unsigned int)video_frame.line_stride_in_bytes;
				...
				if (video_frame.p_data) {
					...
					switch (video_frame.FourCC) {
							...
							case NDIlib_FourCC_type_UYVY: // YCbCr color space
								printf("UYVY\n");
								break;
							// Activated with NDIlib_recv_color_format_best
							// Too many unknowns
							// 1) Test patterns does not send metadata
							//    Unknown pixel format (two possible values for HDR: bt_2100_pq or bt_2100_hlg)
							// 2) Data range for 8 bit data buffer > 16bit pixels
							//    Assume 2=16-235 scaled up
							//    Convert to 0.0-1.0 range for floating point
							case NDIlib_FourCC_video_type_P216:
							{
								...
								// This is identical to a 16bpp version of the NV16 format.
								...
								// Y0 Y1 Y2 Y3 // WORDS
								// U0 V0 U2 V2 // DWORDS LSW - U value MSW - V value
								//

								int ndiWidth = video_frame.xres;
								int ndiHeight = video_frame.yres;
								int ndiLineStrideInBytes = video_frame.line_stride_in_bytes;

								printf("%dx%d (stride %d)\n", ndiWidth, ndiHeight, ndiLineStrideInBytes);
								// P216
								// 1920x1080 (stride 3840) 1920x2
								...
								// The first buffer is a 16bpp luminance buffer
								// Y0 Y1 Y2 Y3 // WORDS
								uint8_t* pSrcY = video_frame.p_data;
								// Immediately after this is an interleaved buffer of 16bpp Cb, Cr pairs.
								// U0 V0 U2 V2 // DWORDS LSW - U value MSW - V value
								uint8_t* pSrcUV = pSrcY + (ndiLineStrideInBytes*ndiHeight);

								// break;


								// the Y component is in the range of [0 . . . 1] (0 being black, 1 being white)
								// and the chroma components range from [−0.5 . . . 0.5]

								// 
								// [0007]
								//  When Y′Pb′Pr′ is stored in an 8-bit approximation,
								//  the Y′ range[0 . . . 1] is mapped to the range 16 . . . 235.
								//  The chroma components Pb′ and Pr′ are mapped to their corresponding
								//  Cb′ and Cr′ components by mapping the range of[−0.5 . . . 0.5]
								//  to an interval of width 224 centered about 128 (i.e. 16 to 240).
								//  For example, an 8-bit representation is computed as :
								//   Y′=16+Y′*219
								// 	 Cb′=128+Pb′*224
								// 	 Cr′=128+Pr′*224
								// [0008] 
								//  Higher bit precisions are computed by simply scaling up the 8-bit values.
								//  For example, an n-bit representation is computed as :
								//   Y′=(16+Y′*219)*2^n−8
								// 	 Cb′=(128+Pb′*224)*2^n−8
								// 	 Cr′=(128+Pr′*224)*2^n−8
								//
								// 16 . . . 235 -> 16*2^n−8 . . . 2352^n−8
								// 16bit : 16x4 ... 235*4 (64 - 940)
								// 256 -> 1024
								// 255 -> 1020
								// 235/255  = 0.9215686274509804
								// 940/1020 = 0.9215686274509804

								// Destination is a 16 bit float buffer
								// Spout format will be R16G16B16A16_FLOAT
								float* pDst = new float ((float)ndiWidth*(float)ndiHeight*4.0f); // RGBA

								uint8_t* srcY  = pSrcY;
								uint8_t* srcUV = pSrcUV;
								uint16_t Y0, Y1, Y2, Y3;
								uint16_t U0, V0, U2, V2;
								float* dst = pDst;
								float r, g, b; // r0, g0, b0, r1, g1, b1, r2, g2, b2, r3, g3, b3;

								// 8bit pointers, 16 bit pixels
								...
								for (int line = 0; line < ndiHeight; line++) {
									for (int col = 0; col <ndiWidth/2; col++) {

										/*
										Y0 = *srcY;	// unsigned 16 bit pixels
										Y1 = *(srcY+2);
										Y2 = *(srcY+4);
										Y3 = *(srcY+5);
										srcY+=8;
										...
										*/
										...
										float y0 = (float)Y0/255.0f;
										float y1 = (float)Y1/255.0f;
										float y2 = (float)Y2/255.0f;
										float y3 = (float)Y3/255.0f;

										float u0 = (float)U0/255.0f;
										float v0 = (float)V0/255.0f;
										float u2 = (float)U2/255.0f;
										float v2 = (float)V2/255.0f;
										...
										/*
										r = 1.16414*(y0-0.06274) - 0.0011*u0 + 1.7923*v0;
										g = 1.16390*(y0-0.06274) - 0.2131*u0 + 0.5342*v0;
										b = 1.16660*(y0-0.06274) - 2.1131*u0 + 0.0001*v0;
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0; // alpha
										...
										r = 1.16414*(y1-0.06274) - 0.0011*u0 + 1.7923*v0;
										g = 1.16390*(y1-0.06274) - 0.2131*u0 + 0.5342*v0;
										b = 1.16660*(y1-0.06274) - 2.1131*u0 + 0.0001*v0;
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0;
										...
										r = 1.16414*(y2-0.06274) - 0.0011*u2 + 1.7923*v2;
										g = 1.16390*(y2-0.06274) - 0.2131*u2 + 0.5342*v2;
										b = 1.16660*(y2-0.06274) - 2.1131*u2 + 0.0001*v2;
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0;
										...
										r = 1.16414*(y3-0.06274) - 0.0011*u2 + 1.7923*v2;
										g = 1.16390*(y3-0.06274) - 0.2131*u2 + 0.5342*v2;
										b = 1.16660*(y3-0.06274) - 2.1131*u2 + 0.0001*v2;
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0;
										...
										*/
										...
										// Y0, U0, V0
										r = 1.16414*((float)Y0-0.06274) - 0.0011*(float)U0 + 1.7923*(float)V0);
										g = 1.16390*((float)Y0-0.06274) - 0.2131*(float)U0 + 0.5342*(float)V0);
										b = 1.16660*((float)Y0-0.06274) - 2.1131*(float)U0 + 0.0001*(float)V0);
										*dst = r; 
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0; // alpha
										...
										// Y1, U0, V0
										r = 1.16414*((float)Y1-0.06274) - 0.0011*(float)U0 + 1.7923*(float)V0);
										g = 1.16390*((float)Y1-0.06274) - 0.2131*(float)U0 + 0.5342*(float)V0);
										b = 1.16660*((float)Y1-0.06274) - 2.1131*(float)U0 + 0.0001*(float)V0);
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0; // alpha
										...
										// Y2, U2, V2
										r = 1.16414*((float)Y2-0.06274) - 0.0011*(float)U2 + 1.7923*(float)V2);
										g = 1.16390*((float)Y2-0.06274) - 0.2131*(float)U2 + 0.5342*(float)V2);
										b = 1.16660*((float)Y2-0.06274) - 2.1131*(float)U2 + 0.0001*(float)V2);
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0; // alpha
										...
										// Y3, U2, V2
										r = 1.16414*((float)Y3-0.06274) - 0.0011*(float)U2 + 1.7923*(float)V2);
										g = 1.16390*((float)Y3-0.06274) - 0.2131*(float)U2 + 0.5342*(float)V2);
										b = 1.16660*((float)Y3-0.06274) - 2.1131*(float)U2 + 0.0001*(float)V2);
										*dst = r;
										*(dst+1) = g;
										*(dst+2) = b;
										*(dst+3) = 1.0; // alpha
								...
								/*
								//
								// https://github.com/NightVsKnight/NDI-Widget/blob/6fb3bf8723711356aa96c3f03bddf94a27b121bf/ndireceiverworker.cpp
								//
								int ndiWidth = video_frame.xres;
								int ndiHeight = video_frame.yres;
								int ndiLineStrideInBytes = video_frame.line_stride_in_bytes;
								int ndiPixelFormat = video_frame.FourCC;
								int ndiArea = ndiWidth * ndiHeight;
								int ndiFrameType = video_frame.frame_format_type;

								// auto pDstY = videoFrame.bits(0);
								// auto pDstUV = videoFrame.bits(1);

								auto pSrcY = video_frame.p_data;
