/*
	...
	08.05.24 - GetPixelData - add unsupported formats
...
unsigned int ofxNDIreceiver::GetSenderStride() {
	return NDIreceiver.GetSenderStride();
}
...
// Only RGBX, RGBA, BGRX, BGRA formats supported
bool ofxNDIreceiver::GetPixelData(ofTexture &texture)
{
	...
	switch (NDIreceiver.GetVideoType()) {
		...
		case NDIlib_FourCC_type_UYVA: // YCbCr using 4:2:2:4
			...
			// LJ DEBUG
			{
				printf("GetPixelData - UYVA\n");
				// not rec
				unsigned char* dest = (unsigned char*)malloc(texture.getHeight()*texture.getWidth()*4);
				ofxNDIutils::YUV422_to_RGBA((const unsigned char*)videoData, dest, (int)texture.getWidth(), (int)texture.getHeight(), (unsigned int)NDIreceiver.GetSenderStride());
				free(dest);
		...
		case NDIlib_FourCC_video_type_YV12: // Planar 8bit 4:2:0 video forma
			printf("GetPixelData - YV12 format not supported\n"); break;
		case NDIlib_FourCC_video_type_I420: // Planar 8bit 4:2:0 video format
			printf("GetPixelData - I420 format not supported\n"); break;
		case NDIlib_FourCC_video_type_NV12: // Planar 8bit 4:2:0 video format
			printf("GetPixelData - NV12 format not supported\n"); break;
