	...
	if (!pNDI_recv) {
		...
		if (p_sources && no_sources > 0) {
			...
			*/
			...
			// LJ DEBUG const NDIlib_tally_t tally_state = { true, false };
			const NDIlib_tally_t tally_state = { false, false };
			p_NDILib->recv_set_tally(pNDI_recv, &tally_state);

			// LJ DEBUG
			// Connect to our sources ???
			// p_NDILib->recv_connect(pNDI_recv, p_sources + index);
		...
		switch (NDI_frame_type) {
			...
			case NDIlib_frame_type_video :
				...
				if (video_frame.p_data) {
					...
					printf("    lastTime [%6.2f] startTime [%6.2f] frametime [%6.2f] msec\n", lastTime, startTime, frametime);
					*/
...
// LJ DEBUG
// Test function
bool ofxNDIreceive::ReceiveTest()
{
	...
	/*
	NDIlib_recv_queue_t pqtotal;
	NDIlib_recv_get_queue(pNDI_recv, &pqtotal);
	// printf("Queue total %ld\n", pqtotal.video_frames);

	NDIlib_recv_performance_t ptotal;
	NDIlib_recv_performance_t pdropped;
	NDIlib_recv_get_performance(pNDI_recv, &ptotal, &pdropped);
	// printf("Total %ld, dropped %ld\n", ptotal.video_frames, pdropped.video_frames);
	*/


	switch (p_NDILib->recv_capture_v2(pNDI_recv, &video_frame, &audio_frame, nullptr, 5000))
	{	// No data
	case NDIlib_frame_type_none:
	...
	// Capture frame frate is half that specified by the connected sender
	// const std::chrono::steady_clock::time_point endTime = std::chrono::steady_clock::now();
	// double elapsed = static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime).count());
	// printf("    elapsed [%6.2f] msec\n", elapsed / 1000.0);
	// printf("Sender fps [%4.2f] : captured fps [%4.2f]\n",
...
bool ofxNDIreceive::ReceiveTest(unsigned int &width, unsigned int &height)
{
	// The descriptors
	// NDIlib_video_frame_v2_t video_frame;
	// NDIlib_audio_frame_v3_t audio_frame;
	...
	if (pNDI_recv) {
		...
		switch (p_NDILib->recv_capture_v3(pNDI_recv, &video_frame, &audio_frame, &metadata_frame, 5000))
		{
			// No data
		case NDIlib_frame_type_none:
			printf("No data received.\n");
