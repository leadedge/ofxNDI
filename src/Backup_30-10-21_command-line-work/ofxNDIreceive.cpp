...
bool ofxNDIreceive::FindSenders(int &nSenders)
{
	...
	if (pNDI_find) {
		...
		// printf("FindSenders : nsources = %d, no_sources = %d\n", nsources, no_sources);

		// If there are new sources and the number has changed
		if (p_sources && nsources != no_sources) {

			printf("FindSenders : senderNameSetup [%s]\n", senderNameSetup.c_str());
			printf("FindSenders : sources changed from %d to %d\n", no_sources, nsources);
			...
			if (no_sources > 0) {
				printf("no_sources > 0 - rebuild sender list\n", no_sources, nsources);
			...
			if (!senderName.empty()) {
				...
				printf("Reset the current sender index\n");
				// LJ DEBUG
				senderIndex = 0;
				if (NDIsenders.size() > 0) {
					// senderIndex = 0;
				...
				// LJ DEBUG
				printf("New sender index (%d) \n", senderIndex);
				printf("    senderNameSetup [%s]\n", senderNameSetup.c_str());

				// LJ DEBUG
				if (!senderNameSetup.empty()) {
					printf("Sender name specified [%d]\n", senderNameSetup);
					// Sender name has been specified to connect to
					// Signal a new sender if it is not the same one as currently selected
					// The calling application can then query this
					if (senderNameSetup != NDIsenders.at(senderIndex)) {
						// LJ DEBUG
						ReleaseReceiver();
...
bool ofxNDIreceive::SetSenderIndex(int index)
{
	...
	// LJ DEBUG
	// ??? 
	// Clear any set receiving name
	senderNameSetup.clear();
...
bool ofxNDIreceive::CreateReceiver(NDIlib_recv_color_format_e colorFormat , int userindex)
{
	...
	int index = userindex; // Can be -1

	// LJ DEBUG
	printf("CreateReceiver : passed index = %d\n", index);
	...
	if (!pNDI_recv) {
		...
		if (p_sources && no_sources > 0) {
			...
			// LJ DEBUG
			// If a sender name has been specified to connect to
			// find it's index in the sources list
			int setindex = -1;
			if (!senderNameSetup.empty()) {
				printf("    Name specified [%s]\n", senderNameSetup.c_str());
				for (int i = 0; i < (int)no_sources; i++) {
					if (p_sources[i].p_ndi_name && p_sources[i].p_ndi_name[0]) {
						// Compare with the requested name
						printf("    Name %d [%s]\n", i, p_sources[i].p_ndi_name);
						if (senderNameSetup == p_sources[i].p_ndi_name) {
			...
			//
			// Now we have the required index
			//

			// If a setup name is specified return and wait if no match
			if (!senderNameSetup.empty() && setindex < 0)
				return false;

			// Otherwise set the required index
			index = setindex;

			printf(" used index = %d\n", index);

			// Quit if the user index is greater than the number of sources
			// LJ DEBUG if (userindex > (int)no_sources - 1)
			if (index > (int)no_sources - 1) {
				printf("    index (%d) > n sources (%d)\n", index, no_sources);
				return false;
			}

			// If index has been specified (-1), use the currently set index
			// LJ DEBUG if (userindex < 0)
			if (index < 0) {
				printf("    using the current index (%d)\n", senderIndex);
				index = senderIndex;
			}
			...
			printf("  sender name at %d set to [%s]\n", index, senderName.c_str());
...
void ofxNDIreceive::ReleaseReceiver()
{
	...
	// LJ DEBUG
	printf("ofxNDIreceive::ReleaseReceiver()\n");
...
//---------------------------------------------------------
bool ofxNDIreceive::SetReceiverName(const char * SenderName)
{
	if (SenderName && SenderName[0]) {
		// Connect to the specified sender
		senderNameSetup = SenderName;
		senderName = SenderName;
	}
	else {
		// Connect to the first sender
		senderNameSetup.clear();
		senderName.clear();
	}
	...
	int index = 0; // index in the list of senders
	if (GetSenderIndex(SenderName, index)) {
		// Set the index as current
		return SetSenderIndex(index);
	}
	return false;
	*/
...
//---------------------------------------------------------
// LJ DEBUG
std::string ofxNDIreceive::GetReceiverName()
{
	return senderNameSetup;
}
...
bool ofxNDIreceive::ReceiveImage(unsigned int &width, unsigned int &height)
{
	...
	if (pNDI_recv) {
		...
		switch (NDI_frame_type) {
			...
			case NDIlib_frame_type_video :
				...
				if (video_frame.p_data) {
					...
					// No video data - no sender
					printf("ofxNDIreceive::ReceiveImage : NDIlib_frame_type_video No video data\n");
					bReceiverConnected = false;
				}
		...
		// No video data - no sender
		printf("ofxNDIreceive::ReceiveImage : No video data - no sender\n");
		bReceiverConnected = false;
	}
	...
	uint32_t timeout_in_ms)
{
	...
	if ((!timeout_in_ms) || (p_NDILib->find_wait_for_sources(p_instance, timeout_in_ms))) {
		...
		// LJ DEBUG
		// uint32_t nsources = 0;
		// p_NDILib->find_get_current_sources(p_instance, &nsources);
		// printf("nsources = %d\n", nsources);
		// *p_no_sources = nsources;
