...
bool ofxNDIreceiver::OpenReceiver()
{
	...
	if (nSenders > 0) {
		...
		if (NDIreceiver.SenderSelected()) {
			...
			// LJ DEBUG
			printf("OpenReceiver : user changed the sender index\n");
			...
			// LJ DEBUG
			// Disable the set receiver name
			// Done in SetSenderIndex ??
			SetReceiverName();
		...
		if (NDIreceiver.ReceiverCreated()) {
			// LJ DEBUG
			// printf("OpenReceiver : Receiver already created\n");
			return true;
		}
...
//---------------------------------------------------------
// LJ DEBUG
bool ofxNDIreceiver::SetReceiverName(const char * SenderName)
{
	return NDIreceiver.SetReceiverName(SenderName);
}

//---------------------------------------------------------
// LJ DEBUG
std::string ofxNDIreceiver::GetReceiverName()
{
	return NDIreceiver.GetReceiverName();
}
