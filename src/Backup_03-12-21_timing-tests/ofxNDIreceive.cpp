...
bool ofxNDIreceive::ReceiveImage(unsigned int &width, unsigned int &height)
{
	...
	if (pNDI_recv) {
		...
		// NDI_frame_type = p_NDILib->recv_capture_v2(pNDI_recv, &video_frame, &audio_frame, &metadata_frame, 0);
		// Vers 4.5
		NDI_frame_type = p_NDILib->recv_capture_v3(pNDI_recv, &video_frame, &audio_frame, &metadata_frame, 670); // 40 frames timeout
...
/*
#ifdef USE_CHRONO
	double frametime = ofxNDIutils::EndTiming(); // msec
	ofxNDIutils::StartTiming();
#else
*/
	lastTime = startTime;
